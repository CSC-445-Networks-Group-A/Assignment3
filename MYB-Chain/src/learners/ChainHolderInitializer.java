...
public class ChainHolderInitializer {
    ...
    public static void main(String args[]) throws IOException, ClassNotFoundException, InterruptedException, NoSuchAlgorithmException {
        ...
        if(userFile.exists() && privateFile.exists() && publicFile.exists()){
            ...
            user = new User(userFileName, privateFileName, publicFileName, blockChainFileName, "ChainHolder", randomNumber.toString(), 10000.0);
            user.persist();
            System.out.println(user.getFullName());
        ...
        ChainHolder chainHolder = new ChainHolder(user);
        chainHolder.start();
        user.start();
        chainHolder.join();
        user.join();
